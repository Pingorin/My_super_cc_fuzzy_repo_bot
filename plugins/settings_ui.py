...
# ✅ SAFE IMPORTS: Agar koi library missing hui toh crash nahi hoga
try:
    import aiohttp
except ImportError:
    pass
...
# 🚀 Ye line terminal me print hogi jab bot start hoga, check karne ke liye ki file load hui ya nahi
print("✅ settings_ui.py Module Loaded Successfully!")
...
# ==============================================================================
@Client.on_message(filters.command(["settings", "setting", "set"]))
async def settings_command(client, message):
    try:
        # User ID nikalna (Anonymous admin ke liye bhi safe)
...
async def main_settings_menu(client, query):
    ...
    buttons = [
        [InlineKeyboardButton("💰 Earning method", callback_data=f"set_earn#{chat_id}"), InlineKeyboardButton("📢 Force Subscribe", callback_data=f"fsub_menu#{chat_id}")],
        [InlineKeyboardButton("📜 Result mode", callback_data=f"set_res_mode#{chat_id}"), InlineKeyboardButton("📄 Result per page", callback_data=f"set_page_limit#{chat_id}")],
        [InlineKeyboardButton("🗑️ Auto-Delete", callback_data=f"autodel_menu#{chat_id}"), InlineKeyboardButton("👍 Auto Reaction", callback_data=f"autoreact_ui#{chat_id}")],
        [InlineKeyboardButton("👋 Welcome Settings", callback_data=f"welcome_ui#{chat_id}"), InlineKeyboardButton("🛡️ Anti-Spam", callback_data=f"antispam_ui#{chat_id}")],
        [InlineKeyboardButton("📢 Auto Post", callback_data=f"autopost_ui#{chat_id}"), InlineKeyboardButton("📣 Auto Mention", callback_data=f"automention_ui#{chat_id}")],
        [InlineKeyboardButton("👑 Admin Free Access", callback_data=f"adm_access_ui#{chat_id}"), InlineKeyboardButton("📊 Daily Stats", callback_data=f"daily_stats#{chat_id}#today")],
        [InlineKeyboardButton("🧨 Reset Settings", callback_data=f"reset_grp_ui#{chat_id}"), InlineKeyboardButton("🔗 Other URLs", callback_data=f"other_urls_ui#{chat_id}")],
        [InlineKeyboardButton("💎 Free Premium", callback_data=f"ref_sys_menu#{chat_id}"), InlineKeyboardButton("💡 Request Features", callback_data=f"req_feature#{chat_id}")],
...
async def set_welcome_photo(client, query):
    ...
    if current_photo:
        await query.message.reply_photo(photo=current_photo, caption="👆 **Current Welcome Photo**")
...
async def reset_welcome(client, query):
    chat_id = int(query.data.split("#")[1])
    await db.update_group_settings(chat_id, {'custom_welcome_text': None, 'custom_welcome_photo': None})
...
async def ap_set_button_step1(client, query):
    ...
    try:
        ...
        await query.message.edit_text(f"✅ Button text set to **{btn_name}**.\n\nNow, please send the **Full URL** for this button.", reply_markup=InlineKeyboardMarkup(cancel_btn))
...
async def admin_access_ui(client, query):
    ...
    buttons = []
    if can_enable:
        if is_enabled: buttons.append([InlineKeyboardButton("Disable Admin Access", callback_data=f"adm_acc_toggle#{chat_id}#off")])
        else: buttons.append([InlineKeyboardButton("Enable Admin Access", callback_data=f"adm_acc_toggle#{chat_id}#on")])
    else:
...
async def daily_stats_ui(client, query):
    ...
    if date_param == "today": target_date = today
    else: target_date = datetime.datetime.strptime(date_param, "%Y-%m-%d")
    ...
    shortener_text = ""
    if not shortener_data: shortener_text = "  - No data available."
    else:
        for safe_domain, data in shortener_data.items():
            ...
            shortener_text += f"  - **{real_domain}**\n    - Generated Links: {gen}\n    - Verified Links: {ver}\n    - Success Ratio: {ratio}%\n"

    text = (
        f"📊 **Daily Stats for {display_date}**\n_(Group ID: {chat_id})_\n\n"
        f"🔍 **Search Statistics:**\n  - Total Requests: {req}\n  - Successful: {suc}\n  - Failed: {failed}\n  - Success Ratio: {search_ratio}%\n\n"
...
async def request_fsub_menu(client, query):
    ...
    s1, s2, s4 = fsub.get('1'), fsub.get('2'), fsub.get('4')
    name1, name2, name4 = await get_name(s1), await get_name(s2), await get_name(s4)

    text = (
        f"⚙️ **Configure Request F-Sub Channels for:** `{chat_id}`\n\n"
        f"1️⃣ **Slot 1:** `{name1}`\n2️⃣ **Slot 2:** `{name2}`\n4️⃣ **Slot 4:** `{name4}` (Post-Verify)"
    )
    buttons = []
    if s1: buttons.append([InlineKeyboardButton("✏️ Edit Slot 1", callback_data=f"set_fsub#{chat_id}#1"), InlineKeyboardButton("🗑️ Clear 1", callback_data=f"clr_fsub#{chat_id}#1")])
    else: buttons.append([InlineKeyboardButton("➕ Set Slot 1", callback_data=f"set_fsub#{chat_id}#1")])
    if s2: buttons.append([InlineKeyboardButton("✏️ Edit Slot 2", callback_data=f"set_fsub#{chat_id}#2"), InlineKeyboardButton("🗑️ Clear 2", callback_data=f"clr_fsub#{chat_id}#2")])
    else: buttons.append([InlineKeyboardButton("➕ Set Slot 2", callback_data=f"set_fsub#{chat_id}#2")])
    if s4: buttons.append([InlineKeyboardButton("✏️ Edit Slot 4", callback_data=f"set_fsub#{chat_id}#4"), InlineKeyboardButton("🗑️ Clear 4", callback_data=f"clr_fsub#{chat_id}#4")])
...
async def normal_fsub_menu(client, query):
    ...
    async def get_name(id_val):
        if not id_val: return "Not Set ❌"
        if isinstance(id_val, str) and ("t.me" in id_val or "http" in id_val): return f"Link Set ({id_val})"
        try:
            chat = await client.get_chat(id_val)
            return f"{chat.title} ({id_val})"
        except: return f"Unknown ({id_val})"

    s3, s5 = fsub.get('3'), fsub.get('5') 
    name3, name5 = await get_name(s3), await get_name(s5)

    text = f"⚙️ **Configure Normal F-Sub for:** `{chat_id}`\n\n3️⃣ **Slot 3:** `{name3}`\n5️⃣ **Slot 5:** `{name5}`"
    buttons = []
    if s3: buttons.append([InlineKeyboardButton("✏️ Edit Slot 3", callback_data=f"set_fsub#{chat_id}#3"), InlineKeyboardButton("🗑️ Clear 3", callback_data=f"clr_fsub#{chat_id}#3")])
    else: buttons.append([InlineKeyboardButton("➕ Set Slot 3", callback_data=f"set_fsub#{chat_id}#3")])
    if s5: buttons.append([InlineKeyboardButton("✏️ Edit Slot 5", callback_data=f"set_fsub#{chat_id}#5"), InlineKeyboardButton("🗑️ Clear 5", callback_data=f"clr_fsub#{chat_id}#5")])
...
async def set_fsub_input(client, query):
    ...
    txt = f"🆔 **Set Slot {slot}**\n\n1. Add bot as Admin in Channel & Forward Message.\n2. Send Channel ID.\n⚠️ **Note:** Bot MUST be Admin!"
    await query.message.edit_text(txt, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Cancel", callback_data=back_cb)]]))
    ...
    if msg.text or msg.forward_from_chat:
        try:
            input_identifier = msg.forward_from_chat.id if msg.forward_from_chat else msg.text.strip()
            if slot == '5' and isinstance(input_identifier, str) and ("t.me/" in input_identifier or "http" in input_identifier):
                ...
                if me.status != enums.ChatMemberStatus.ADMINISTRATOR:
                    return await msg.reply(f"❌ **Error:** I am not an Admin in **{real_chat.title}**!")
                await db.update_fsub_channel(chat_id, slot, real_chat.id)
                await msg.reply(f"✅ **Saved!** ID: `{real_chat.id}`")
...
async def remove_req_all(client, query):
    chat_id = int(query.data.split("#")[1])
    for s in ['1', '2', '4']: await db.remove_fsub_channel(chat_id, s)
...
async def remove_norm_all(client, query):
    chat_id = int(query.data.split("#")[1])
    for s in ['3', '5']: await db.remove_fsub_channel(chat_id, s)
...
async def shortlink_config(client, query):
    ...
    if mode == 'dynamic':
        ...
        custom_btns.append([InlineKeyboardButton("⏳ Gap 1", callback_data=f"time_ui#{chat_id}#time_gap1"), InlineKeyboardButton("⏳ Gap 2", callback_data=f"time_ui#{chat_id}#time_gap2")])
        custom_btns.append([InlineKeyboardButton("⏰ Set Full Access", callback_data=f"time_ui#{chat_id}#time_smart")])

    footer_btns = [[InlineKeyboardButton("⚙️ Configure Shorteners", callback_data=f"set_slots#{chat_id}")], [InlineKeyboardButton("🔙 Back", callback_data=f"set_earn#{chat_id}")]]
    await query.message.edit_text(f"🔗 **Shortener Mode Config**", reply_markup=InlineKeyboardMarkup([mode_btns] + custom_btns + footer_btns))
...
async def time_picker_ui(client, query):
    ...
    if "gap" in key:
        buttons = [[InlineKeyboardButton("5 Mins", callback_data=f"save_time#{chat_id}#{key}#{5*60}"), InlineKeyboardButton("15 Mins", callback_data=f"save_time#{chat_id}#{key}#{15*60}")],
                   [InlineKeyboardButton("1 Hour", callback_data=f"save_time#{chat_id}#{key}#{3600}"), InlineKeyboardButton("3 Hours", callback_data=f"save_time#{chat_id}#{key}#{3*3600}")]]
    else:
        buttons = [[InlineKeyboardButton("12 Hours", callback_data=f"save_time#{chat_id}#{key}#{12*3600}"), InlineKeyboardButton("24 Hours", callback_data=f"save_time#{chat_id}#{key}#{86400}")],
                   [InlineKeyboardButton("3 Days", callback_data=f"save_time#{chat_id}#{key}#{3*86400}"), InlineKeyboardButton("7 Days", callback_data=f"save_time#{chat_id}#{key}#{7*86400}")]]
    buttons.append([InlineKeyboardButton("🔙 Back", callback_data=f"set_smode#{chat_id}")])
    await query.message.edit_text("⏱ **Set Time**", reply_markup=InlineKeyboardMarkup(buttons))
...
async def configure_slots(client, query):
    chat_id = int(query.data.split("#")[1])
    group_data = await db.get_group_settings(chat_id)
    shorteners = group_data.get('shorteners', {}) if group_data else {}
    
    text = f"🛠️ **Configuring Setup for:** `{chat_id}`\n\n"
    for i in range(1, 4):
        s_data = shorteners.get(str(i))
        text += f"✅ Shortener {i}: {s_data['site']}\n" if s_data else f"❌ Shortener {i}: Not Set\n"

    buttons = []
    for i in range(1, 4):
        if shorteners.get(str(i)): buttons.append([InlineKeyboardButton(f"✏️ Edit Slot {i}", callback_data=f"edit_slot#{chat_id}#{i}"), InlineKeyboardButton(f"🗑️ Reset {i}", callback_data=f"del_slot#{chat_id}#{i}")])
        else: buttons.append([InlineKeyboardButton(f"➕ Set Shortener {i}", callback_data=f"add_slot#{chat_id}#{i}")])

    footer_btns = [[InlineKeyboardButton("🧪 Test Connections", callback_data=f"test_sl#{chat_id}")], [InlineKeyboardButton("🔙 Back", callback_data=f"set_smode#{chat_id}")]]
...
async def reset_confirm_ui(client, query):
    chat_id = int(query.data.split("#")[1])
    text = "⚠️ **Are you sure?**\n\nThis will **PERMANENTLY DELETE** all settings for this group.\n"
    buttons = [[InlineKeyboardButton("✅ Yes, Reset", callback_data=f"reset_grp_now#{chat_id}")], [InlineKeyboardButton("❌ Cancel", callback_data=f"set_main#{chat_id}")]]
    await query.message.edit_text(text, reply_markup=InlineKeyboardMarkup(buttons))
...
async def reset_group_now(client, query):
    ...
    await query.message.edit_text("✅ **Settings Reset Complete!**", reply_markup=InlineKeyboardMarkup(btn))
...
async def other_urls_ui(client, query):
    ...
    text = f"🔗 **Other URL & Button Settings for:** `{chat_id}`\n\n"
    buttons = [
...
async def set_caption_url_handler(client, query):
    ...
    await query.message.edit_text("Please send the URL to be used for the file name hyperlink.\nSend `remove` to delete.", reply_markup=InlineKeyboardMarkup(cancel_btn))
    try:
        msg = await client.listen(chat_id=query.message.chat.id, user_id=query.from_user.id, timeout=60)
        if msg.text:
            if msg.text.lower() == "remove": await db.update_group_settings(chat_id, {'caption_url': None})
            elif "http" in msg.text: await db.update_group_settings(chat_id, {'caption_url': msg.text.strip()})
...
async def caption_btn_ui(client, query):
    chat_id = int(query.data.split("#")[1])
    buttons = [[InlineKeyboardButton("Set Text", callback_data=f"set_cb_txt#{chat_id}"), InlineKeyboardButton("Set URL", callback_data=f"set_cb_url#{chat_id}")],
               [InlineKeyboardButton("Reset Button", callback_data=f"reset_cb#{chat_id}")], [InlineKeyboardButton("🔙 Back", callback_data=f"other_urls_ui#{chat_id}")]]
    await query.message.edit_text(f"✨ **Caption Button Settings for:** `{chat_id}`", reply_markup=InlineKeyboardMarkup(buttons))

@Client.on_callback_query(filters.regex(r"^set_cb_txt#"))
async def set_cap_btn_text(client, query):
    chat_id = int(query.data.split("#")[1])
    await query.message.edit_text("Please send the **text** for your custom button.")
    try:
        msg = await client.listen(chat_id=query.message.chat.id, user_id=query.from_user.id, timeout=60)
        if msg.text: await db.update_group_settings(chat_id, {'caption_btn_text': msg.text})
...
async def set_cap_btn_url(client, query):
    chat_id = int(query.data.split("#")[1])
    await query.message.edit_text("Please send the **full URL**.")
    try:
        msg = await client.listen(chat_id=query.message.chat.id, user_id=query.from_user.id, timeout=60)
        if msg.text and "http" in msg.text: await db.update_group_settings(chat_id, {'caption_btn_url': msg.text.strip()})
        await caption_btn_ui(client, query)
    except: pass
...
async def set_howto_url_handler(client, query):
    chat_id = int(query.data.split("#")[1])
    await query.message.edit_text("Please send the new URL for the 'How to Download' button.\nSend `remove` to delete.")
    try:
        msg = await client.listen(chat_id=query.message.chat.id, user_id=query.from_user.id, timeout=60)
        if msg.text:
            if msg.text.lower() == "remove": await db.update_group_settings(chat_id, {'howto_url': None})
            elif "http" in msg.text: await db.update_group_settings(chat_id, {'howto_url': msg.text.strip()})
            await other_urls_ui(client, query)
    except: pass
...
async def set_group_link_handler(client, query):
    chat_id = int(query.data.split("#")[1])
    await query.message.edit_text("Please send the invite link for your group.\nSend `remove` to delete.")
    try:
        msg = await client.listen(chat_id=query.message.chat.id, user_id=query.from_user.id, timeout=60)
        if msg.text:
            if msg.text.lower() == "remove": await db.update_group_settings(chat_id, {'group_link': None})
            elif "t.me" in msg.text or "http" in msg.text: await db.update_group_settings(chat_id, {'group_link': msg.text.strip()})
            await other_urls_ui(client, query)
    except: pass
...
async def referral_settings_menu(client, query):
    ...
    buttons = [
        ...
        [InlineKeyboardButton("Set Claim Points", callback_data=f"ref_set_points#{chat_id}"), InlineKeyboardButton("Set Access Time", callback_data=f"ref_set_time#{chat_id}")],
        [InlineKeyboardButton("🔙 Back to Main Settings", callback_data=f"set_main#{chat_id}")]
    ]
    await query.message.edit_text(f"🤝 **Referral System Settings for:** `{chat_id}`", reply_markup=InlineKeyboardMarkup(buttons))
...
async def referral_set_points_ui(client, query):
    chat_id = int(query.data.split("#")[1])
    buttons = [[InlineKeyboardButton("3", callback_data=f"ref_save_pts#{chat_id}#3"), InlineKeyboardButton("5", callback_data=f"ref_save_pts#{chat_id}#5")],
               [InlineKeyboardButton("10", callback_data=f"ref_save_pts#{chat_id}#10"), InlineKeyboardButton("20", callback_data=f"ref_save_pts#{chat_id}#20")],
               [InlineKeyboardButton("🔙 Back", callback_data=f"ref_sys_menu#{chat_id}")]]
    await query.message.edit_text("🔢 **Set Referral Target**", reply_markup=InlineKeyboardMarkup(buttons))
...
async def referral_set_time_ui(client, query):
    chat_id = int(query.data.split("#")[1])
    buttons = [[InlineKeyboardButton("1 D", callback_data=f"ref_save_time#{chat_id}#{1*86400}"), InlineKeyboardButton("7 D", callback_data=f"ref_save_time#{chat_id}#{7*86400}")],
               [InlineKeyboardButton("15 D", callback_data=f"ref_save_time#{chat_id}#{15*86400}"), InlineKeyboardButton("30 D", callback_data=f"ref_save_time#{chat_id}#{30*86400}")],
               [InlineKeyboardButton("🔙 Back", callback_data=f"ref_sys_menu#{chat_id}")]]
    await query.message.edit_text("⏳ **Set Reward Duration**", reply_markup=InlineKeyboardMarkup(buttons))
...
async def request_feature_ui(client, query):
    chat_id = int(query.data.split("#")[1])
    await query.message.edit_text("💡 **Request a New Feature**\nPlease describe the feature.")
    try:
        msg = await client.listen(chat_id=query.message.chat.id, user_id=query.from_user.id, timeout=60)
        ...
        for admin_id in ADMINS:
            try:
                ...
                if msg.text: await client.send_message(admin_id, msg.text)
            except: pass
...
async def back_to_group_list(client, query):
    try:
        ...
        if not user_groups: return await query.message.edit_text("❌ **No Groups Found!**")
        buttons = [[InlineKeyboardButton(f"📂 {title}", callback_data=f"set_main#{chat_id}")] for title, chat_id in user_groups]
