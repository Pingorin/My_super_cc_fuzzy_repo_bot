...
# Ye function check karega ki agla kaunsa step bacha hai
# Jo level info.py me set nahi hai, use SKIP kar dega automatically.
async def get_next_verification(user_id, chat_id):
    if not info.IS_VERIFY:
        return None # Verification OFF hai

    # Check Level 1 (Agar Config me set hai)
    ...
    # Check Level 2 (Agar Config me set hai)
    ...
    # Check Level 3 (Agar Config me set hai)
...
async def start_handler(client, message):
    ...
    if len(message.command) > 1 and message.command[1].startswith("verify_"):
        try:
            ...
            # Agar Agla Step Bacha hai -> Link bhejo
            if next_step:
                lvl = next_step['level']
                site = next_step['site']
                api = next_step['api']
...
@Client.on_message(filters.command("set_shortner") & filters.user(info.ADMINS))
