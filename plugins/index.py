...
# ====================================================
#  COMMAND: /delete_all (Reset Database)
# ====================================================
@Client.on_message(filters.command("delete_all") & filters.user(ADMINS), group=-1)
async def delete_database_handler(bot, message):
    # Debugging ke liye print lagaya hai
    print(f"Delete command received from: {message.from_user.id}")
        ...
        try:
            if message.forward_from_chat:
                ...
                await message.reply_text(
                    ...
                    f"(Agar shuru se scan karna hai to **0** bhejein)."
                )
            else:
...
async def start_index(bot, query):
    ...
    # Stats Tracking (Start from Skip count)
    stats = {
        'saved': skip,
        'new': 0,
        'dup': 0,
    ...
    try:
        while current <= last_id:
            ...
            for m in msgs:
                ...
                if media:
                    # Pass message 'm' for optimized DB
